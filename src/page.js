...
<script>
  ...
  // ── Client-side ZIP utilities & chunked upload engine ───────────────────────
  // 以下函数（_protoDetectEntry/_protoFixEnc/_protoStrip/_protoFilter/_fnv1a/
  // _protoPlanBatches/_protoChunkedUpload）由服务端注入的共享引擎提供，
  // 见 src/proto-upload-engine.js（与管理后台共用同一份实现）。
  ...
  document.getElementById('protoUploadBtn').addEventListener('click', async () => {
    ...
    try {
      const result = await _protoChunkedUpload({
        ...
        setStatus,
        unzip: _unzipBytes, authToken: token, fetchImpl: fetch,
