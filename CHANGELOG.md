...
### Added
- **上传保留原始文件名**：`key` 仍为随机串（直链路径不变），原名额外存入 KV `imgmeta` value、`userimgs` 条目与 R2 对象自定义元数据
...
- `scripts/typora-upload.sh` 与 `scripts/onimg-cli.py`（含 Worker 内嵌副本）上传时自动带上原名
...
- **一键安装脚本** `GET /install.sh`：Worker 内嵌脚本（注入当前 origin），解决私有 repo 无法访问 raw.githubusercontent.com 的问题
...
- Python `urllib` 默认 UA 被 Cloudflare Bot 防护 error 1010 拦截，全部请求加 Chrome UA
...
### Added
- **审计日志后端**：KV 按天滚动写入（`audit:{day}`，cap 500，TTL 92 天），路由层集中埋点，记录 actor / target / status / IP
- `GET /admin/audit` 查询接口（支持日期/操作类型/actor 过滤）
- 覆盖范围：登录、删除、彻底清除、用户增删改、配置变更
- 新增 8 项单测（共 93 项）

### Known Issues
- 后台审计 UI 与访问趋势折线图**未落地**——admin-page.js 内联模板编辑反复失败致后台崩溃，已回退基线
