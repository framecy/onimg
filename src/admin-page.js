...
<div class="modal-overlay" id="statsModal">
  <div class="modal stats-modal">
    ...
    <div class="modal-body">
      ...
      <div class="country-bars"><h4>最近 14 天访问趋势</h4><div style="height:160px;position:relative"><canvas id="imgTrendChart"></canvas></div></div>
      <div class="access-log">
        <h4>最近访问记录</h4>
        <table class="log-table">
          <thead><tr><th>时间</th><th>IP</th><th>国家</th><th>城市</th><th>ISP</th></tr></thead>
...
<div class="modal-overlay" id="protoStatsModal">
  <div class="modal stats-modal">
    ...
    <div class="modal-body">
      ...
      <div class="country-bars"><h4>最近 14 天访问趋势</h4><div style="height:160px;position:relative"><canvas id="protoTrendChart"></canvas></div></div>
      <div class="access-log">
        <h4>最近访问记录</h4>
        <table class="log-table">
...
<script>
  ...
  async function loadAudit() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--tx-3);padding:24px">加载中…</td></tr>';
    try {
      const res = await fetch('/admin/audit?days=' + encodeURIComponent(days), { headers: authH() });
      if (res.status === 401) { handleUnauth(); return; }
      if (!res.ok) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--tx-3);padding:24px">加载失败</td></tr>'; return; }
      auditEntries = (await res.json()).entries || [];
      renderAudit();
    } catch { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--tx-3);padding:24px">加载失败</td></tr>'; }
  }
  function renderAudit() {
    const tbody = document.getElementById('auditBody');
    if (!auditEntries.length) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--tx-3);padding:24px">暂无记录</td></tr>'; return; }
    tbody.innerHTML = auditEntries.map(e => {
      ...
      return '<tr><td style="white-space:nowrap;font-family:var(--mono);font-size:.78rem">'+fmtTs(e.ts)+'</td>'+
        ...
        '<td style="color:'+(ok?'var(--green)':'var(--red)')+';font-weight:600">'+(ok?'成功':'失败')+'</td>'+
  ...
  function exportAuditCsv() {
    ...
    const rows = auditEntries.map(e => [fmtTs(e.ts), AUDIT_ACTION_LABEL[e.action]||e.action, e.actor||'', e.target||'', e.status==='fail'?'失败':'成功', e.ip||'']);
  ...
  function ensureChartJs() {
    ...
    _chartLoading = new Promise(resolve => {
      ...
      sc.onerror = () => { _chartLoading = null; resolve(false); };
  ...
  const _trendCharts = {};
  async function renderTrendChart(canvasId, accesses, days) {
    days = days || 14;
    ...
    const agg = aggregateDailyHits(accesses, days);
    if (_trendCharts[canvasId]) _trendCharts[canvasId].destroy();
    _trendCharts[canvasId] = new window.Chart(canvas, {
      type: 'line',
      data: { labels: agg.labels, datasets: [{ data: agg.data, borderColor:'#34d399', backgroundColor:'rgba(52,211,153,.12)', fill:true, tension:.3, pointRadius:2, borderWidth:2 }] },
      options: {
